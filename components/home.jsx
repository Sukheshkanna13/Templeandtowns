...
// ---------- Search Pill + Bottom Sheet (mobile) ----------
const SearchPillMobile = ({ city, checkIn, checkOut, guests, setCity, setCheckIn, setCheckOut, setGuests, onSubmit }) => {
  ...
  const guestShort   = guests.split(' · ')[0];
  const dateRange    = checkIn && checkOut
    ? `${fmtShort(checkIn)} – ${fmtShort(checkOut)}`
    : 'Add dates';
  const pillText     = `${city}  ·  ${dateRange}  ·  ${guestShort}`;

  const sheet = (
    <div className="tt-sheet-backdrop" onClick={() => setOpen(false)}>
      <div className="tt-bottom-sheet" onClick={e => e.stopPropagation()}>
        <div className="tt-sheet-drag-wrap">
          <div className="tt-sheet-drag-bar" />
        </div>
        <p style={{ margin: '0 0 20px', fontSize: 17, fontWeight: 600, color: 'var(--ink)' }}>
          Search stays
        </p>
  ...
  return (
    <div className="tt-pill-wrap">
      <button className="tt-search-pill" onClick={() => setOpen(true)} aria-label="Search stays">
        <span className="tt-pill-label">{pillText}</span>
        <span className="tt-pill-icon"><Ico name="search" size={15} /></span>
      </button>
  ...
  return (
    <>
      ...
      <section
        ...
        {/* Dark overlay — hidden on desktop, shown on mobile via CSS */}
        ...
        <div className="tt-page">
          <div className="tt-hero-grid" style={{ alignItems: 'center' }}>
            ...
            {/* LEFT — headline */}
            <div className="tt-hero-text-wrap">
              ...
              {/* Hidden on mobile — pill replaces these */}
              <div className="tt-hero-cta-btns" style={{ display: 'flex', gap: 14, marginTop: 36, flexWrap: 'wrap' }}>
            ...
            <div className="tt-featured-card" style={{ aspectRatio: 'unset', height: 280 }}>
              ...
              <div style={{
                ...
                background: 'linear-gradient(180deg, rgba(10,22,40,0.35) 0%, rgba(10,22,40,0.05) 40%, rgba(10,22,40,0.55) 100%)',
              }} />
        ...
        {/* SEARCH PILL — mobile only (wrapper controls visibility) */}
        <div className="tt-pill-mobile-wrap">
          <SearchPillMobile
        ...
        {/* FULL-WIDTH SEARCH BAR — desktop only (hidden on mobile via CSS) */}
        <div className="tt-search-desktop-wrap" style={{ marginTop: 40, borderTop: '1px solid var(--line)' }}>
      ...
      {/* ===== FEATURED PROPERTIES (filtered by active tab) ===== */}
            ...
            <div className="tt-grid-3">
              {filteredProps.map(p => (
                <div
                  key={p.id}
                  className="tt-card"
                  onClick={() => go('property', { propertyId: p.id })}>
          ...
          <div className="tt-loyalty-teaser" style={{
            ...
            <div>
              ...
              <p style={{
                ...
                lineHeight: 1.55, maxWidth: 480, margin: 0,
              }}>
